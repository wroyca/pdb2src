...
using pair = std::pair<std::wstring, unsigned long>;
using sort_pair = std::pair<unsigned long, std::wstring>;
using local = std::vector<std::wstring>;

std::vector<std::wstring> vdata;

static std::map<std::wstring, std::wstring> cache_fn_body_zy;
static std::map<std::wstring, local> cache_fn_body;
static std::map<sort_pair, std::pair<std::wstring, unsigned long> >
    cache_fn_map;
static std::vector<std::wstring> cache_data;
static std::wstring fn = L"";
static std::wstring fxn = L"";
static std::wstring zy = L"";
static bool break_me = false;
...
CComPtr<IDiaSession> demo_dia_session;
CComPtr<IDiaDataSource> demo_dia_data_source;
CComPtr<IDiaSymbol> demo_dia_symbol;
...
constexpr wchar_t *const rgUdtKind[] = {
  L"struct",
  L"class",
  L"union",
  L"interface",
...
constexpr wchar_t *const rgAccess[] = { L"", // No access specifier
                                        L"private", L"protected", L"public" };

constexpr wchar_t *const rgBaseType[] = {
  L"<NoType>", // btNoType = 0,
  L"void",     // btVoid = 1,
  L"char",     // btChar = 2,
  L"wchar_t",  // btWChar = 3,
...
static bool bound_valid = false;
static std::wstring some_bound = L"";

constexpr wchar_t *const rgLocationTypeString[]
    = { L"NULL",        L"static",       L"TLS",      L"RegRel",
        L"ThisRel",     L"Enregistered", L"BitField", L"Slot",
        L"IL Relative", L"In MetaData",  L"Constant" };

using MapIa64Reg = struct MapIa64Reg
{
  CV_HREG_e iCvReg;
...
const MapIa64Reg mpIa64regSz[] = { { CV_IA64_Br0, L"Br0" },
                                   { CV_IA64_Br1, L"Br1" },
                                   { CV_IA64_Br2, L"Br2" },
                                   { CV_IA64_Br3, L"Br3" },
                                   { CV_IA64_Br4, L"Br4" },
...
static std::wstring f_name = L"";
static std::wstring d_name = L"";

////////////////////////////////////////////////////////////
// Map an IA64 registry ID with the corresponding string name
//
auto
cmpIa64regSz (const void *pv1, const void *pv2) -> int
{
...
auto
PrintUdtKind (IDiaSymbol *pSymbol) -> void
{
  DWORD dwKind = 0;

  if (pSymbol->get_udtKind (&dwKind) == S_OK)
    {
      fn.append (rgUdtKind[dwKind]);
      fn.append (L" ");
      zy.append (rgUdtKind[dwKind]);
      zy.append (L" ");
    }
}

auto
PrintName (IDiaSymbol *pSymbol) -> void
{
  BSTR bstrName;
  BSTR bstrUndName;
  ...
  if (pSymbol->get_undecoratedName (&bstrUndName) == S_OK)
    {
      if (wcscmp (bstrName, bstrUndName) == 0)
        ...
        {
          // fn.append(L"%s(%s)", bstrUndName, bstrName);
        }
...
PrintVariant (VARIANT var) -> void
{
  switch (var.vt)
    ...
    case VT_I1:
      fn.append (L" 0x");
      fn.append (std::to_wstring (var.bVal));

      zy.append (L" 0x");
      zy.append (std::to_wstring (var.bVal));
      break;
    ...
    case VT_BOOL:
      fn.append (L" 0x");
      fn.append (std::to_wstring (var.iVal));

      zy.append (L" 0x");
      zy.append (std::to_wstring (var.iVal));
      break;
    ...
    case VT_ERROR:
      fn.append (L" 0x");
      fn.append (std::to_wstring (var.lVal));
      zy.append (L" 0x");
      zy.append (std::to_wstring (var.lVal));
      break;

    case VT_R4:
      fn.append (L" ");
      fn.append (std::to_wstring (var.fltVal));
      zy.append (L" ");
      zy.append (std::to_wstring (var.fltVal));
      break;

    case VT_R8:
      fn.append (L" ");
      fn.append (std::to_wstring (var.dblVal));
      zy.append (L" ");
      zy.append (std::to_wstring (var.dblVal));
      break;

    case VT_BSTR:
      fn.append (L" \"");
      fn.append (var.bstrVal);
      fn.append (L"\"");
      zy.append (L" \"");
      zy.append (var.bstrVal);
      zy.append (L"\"");
      break;
...
auto
PrintBound (IDiaSymbol *pSymbol) -> void
{
  DWORD dwTag = 0;
  DWORD dwKind;
...
auto
SzNameC7Reg (USHORT reg, DWORD MachineType) -> const wchar_t *
{
  ...
  switch (reg)
    ...
    case CV_ALLREG_LOCALS:
      return L"BaseOfLocals";
    case CV_ALLREG_PARAMS:
      return L"BaseOfParams";
    case CV_ALLREG_VFRAME:
      return L"VFrame";
    }
  // wprintf_s(wszRegNum, L"???(0x%x)", reg);
  switch (MachineType)
    {
    case CV_CFL_8080:
    case CV_CFL_8086:
    ...
    case CV_CFL_IA64:
      {
        ...
        p = (MapIa64Reg *)bsearch (
            &m, mpIa64regSz, sizeof (mpIa64regSz) / sizeof (*mpIa64regSz),
...
auto
SzNameC7Reg (USHORT reg) -> const wchar_t *
{
  return SzNameC7Reg (reg, g_dwMachineType);
}

auto
PrintLocation (IDiaSymbol *pSymbol) -> void
{
  DWORD dwLocType;
  ...
  switch (dwLocType)
    ...
    case LocIsStatic:
      if ((pSymbol->get_relativeVirtualAddress (&dwRVA) == S_OK)
        ...
        {
          // wprintf(L"%s, [%08X][%04X:%08X]", SafeDRef(rgLocationTypeString,
          // dwLocType), dwRVA, dwSect, dwOff);
        }
      break;
    ...
    case LocIsRegRel:
      if ((pSymbol->get_registerId (&dwReg) == S_OK)
        ...
        {
          // wprintf(L"%s Relative, [%08X]", SzNameC7Reg((USHORT) dwReg),
          // lOffset);
        }
      break;
    ...
    case LocIsBitField:
      if ((pSymbol->get_offset (&lOffset) == S_OK)
        ...
        {
          // wprintf(L"this(bf)+0x%X:0x%X len(0x%X)", lOffset, dwBitPos,
          // (ULONG) ulLen);
        }
      break;
    ...
    case LocIsSlot:
      if (pSymbol->get_slot (&dwSlot) == S_OK)
        {
          // wprintf(L"%s, [%08X]", SafeDRef(rgLocationTypeString, dwLocType),
          // dwSlot);
        }
      break;
...
PrintType (IDiaSymbol *pSymbol)
{
  ...
  if (dwTag != SymTagPointerType)
    {
      ...
      if ((pSymbol->get_volatileType (&bSet) == S_OK) && bSet)
        {
          fn.append (L"volatile ");
          zy.append (L"volatile ");
        }

      if ((pSymbol->get_unalignedType (&bSet) == S_OK) && bSet)
        {
          fn.append (L"__unaligned ");
          zy.append (L"__unaligned ");
        }
    }
  ...
  switch (dwTag)
    ...
    case SymTagFunctionType:
      fn.append (L"function ");
      zy.append (L"function ");
      break;
      ...
      if (pSymbol->get_type (&pBaseType) == S_OK)
        {
          ...
          if (pSymbol->get_rank (&dwRank) == S_OK)
            {
              if (SUCCEEDED (pSymbol->findChildren (SymTagDimension, NULL,
                                                    nsNone, &pEnumSym))
                  && (pEnumSym != NULL))
                {
                  while (SUCCEEDED (pEnumSym->Next (1, &pSym, &celt))
                         && (celt == 1))
                    {
                      IDiaSymbol *pBound;
              ...
              if (pSymbol->get_count (&dwCountElems) == S_OK)
                {
                  bound_valid = true;
                  some_bound = L"";
                  some_bound.append (L"[0x");
                  some_bound.append (std::to_wstring (dwCountElems));
                  some_bound.append (L"]");
                }
                  ...
                  if (ulLenElem == 0)
                    {
                      ...
                      some_bound = L"";
                      some_bound.append (L"[0x");
                      some_bound.append (std::to_wstring ((ULONG)ulLenArray));
                      some_bound.append (L"]");
                    }
    ...
    case SymTagBaseType:
      ...
      switch (dwInfo)
        {
        case btUInt:
          fn.append (L"unsigned ");
          zy.append (L"unsigned ");
      ...
      fn.append (rgBaseType[dwInfo]);
      zy.append (rgBaseType[dwInfo]);
      break;
...
static std::wstring lxx = L"";
static std::wstring lxx_zy = L"";
static std::wstring lxx_data = L"";

void
PrintFunctionType (IDiaSymbol *pSymbol)
{
  std::vector<std::wstring> types;
  DWORD dwAccess = 0;
  std::wstring lox = L"";
  zy.append (L"return zynamics::bind<");

  if (pSymbol->get_access (&dwAccess) == S_OK)
    {
      fn.append (SafeDRef (rgAccess, dwAccess));
      zy.append (SafeDRef (rgAccess, dwAccess));
      fn.append (L" ");
      zy.append (L" ");
    }
      ...
      if (pFuncType->get_type (&pReturnType) == S_OK)
        {
          ...
          IDiaEnumSymbols *pEnumParam;
          std::vector<std::wstring> vParam;

          if (SUCCEEDED (pSymbol->findChildren (SymTagData, NULL, nsNone,
                                                &pEnumParam)))
            {
              IDiaSymbol *pParam;
              ULONG celt = 0;

              while (SUCCEEDED (pEnumParam->Next (1, &pParam, &celt))
                     && (celt == 1))
                {
                  wchar_t *name;

                  if (pParam->get_name (&name) == S_OK
                      && wcscmp (name, L"") != 0)
                    {
                      // IDiaSymbol *tt;
                      // if(pParam->get_type(&tt))
                      //{
                      //   PrintType(tt);
                      //   tt->Release();
                      // }
                      DWORD dwDataKind;
                      if (pParam->get_dataKind (&dwDataKind) == S_OK)
                        {
                          std::wstring typl
                              = SafeDRef (rgDataKind, dwDataKind);

                          if (wcscmp (typl.c_str (), L"Param") == 0)
                            {
                              std::wstring vp (name);
                              vParam.emplace_back (vp);
                              SysFreeString (name);
                            }
                          else
                            {
                              std::wstring vp (name);
                              vdata.emplace_back (vp);
                              SysFreeString (name);
                            }
                        }
                    }
          ...
          if (SUCCEEDED (pFuncType->findChildren (SymTagNull, NULL, nsNone,
                                                  &pEnumChildren)))
            {
              IDiaSymbol *pChild;
              ULONG celt = 0;
              ...
              while (SUCCEEDED (pEnumChildren->Next (1, &pChild, &celt))
                     && (celt == 1))
                {
                  IDiaSymbol *pType;
                  if (pChild->get_type (&pType) == S_OK)
                    {
                      ...
                      try
                        {
                          fn.append (vParam.at (nParam - 1).c_str ());
                          // zy.append(vParam.at(nParam -1).c_str());
                        }
                      catch (...)
                        {
                        }
                      ...
                      if (bound_valid == true)
                        {
                          bound_valid = false;
                          fn.append (some_bound);
                          zy.append (some_bound);
                        }
              ...
              zy.append (L")");
              zy.erase (std::remove (zy.begin (), zy.end (), L'\\'),
                        zy.end ());
...
auto
PrintSymTag (DWORD dwSymTag) -> void
{
  // wprintf(L"%-15s: ", SafeDRef(rgTags, dwSymTag));
}

auto
PrintSymbolType (IDiaSymbol *pSymbol) -> void
{
  IDiaSymbol *pType;
...
auto
PrintData (IDiaSymbol *pSymbol) -> void
{
  PrintLocation (pSymbol);
...
auto
PrintUndName (IDiaSymbol *pSymbol) -> void
{
  BSTR bstrName;
      ...
      if (pSymbol->get_name (&bstrName) == S_OK)
        {
          ...
          f_name.append (bstrName[0] != L'\0' ? bstrName : L"(none)");
...
void
PrintCallSiteInfo (IDiaSymbol *pSymbol)
{
...
void
PrintCoffGroup (IDiaSymbol *pSymbol)
{
  DWORD dwISect, dwOffset;
  if (pSymbol->get_addressSection (&dwISect) == S_OK
      && pSymbol->get_addressOffset (&dwOffset) == S_OK)
...
void
PrintHeapAllocSite (IDiaSymbol *pSymbol)
{
  DWORD dwISect, dwOffset;
  if (pSymbol->get_addressSection (&dwISect) == S_OK
      && pSymbol->get_addressOffset (&dwOffset) == S_OK)
...
void
PrintCompilandEnv (IDiaSymbol *pSymbol)
{
  PrintName (pSymbol);
  // wprintf(L" =");
...
void
PrintCompilandDetails (IDiaSymbol *pSymbol)
{
  DWORD dwLanguage;
...
auto
PrintSymbol (IDiaSymbol *pSymbol, DWORD dwIndent) -> void
{
  IDiaSymbol *pType;
  DWORD dwSymTag;
  ULONGLONG ulLen;

  auto found = f_name.find (fxn);
  // putwchar(L'\n');
  ...
  switch (dwSymTag)
    ...
    case SymTagFunction:
    case SymTagBlock:
      if (!firstrun && found != std::string::npos)
        {
          break_me = true;
          break;
        }
  ...
  /*if ((dwSymTag == SymTagUDT) || (dwSymTag == SymTagAnnotation)) {
    ...
    if (SUCCEEDED(pSymbol->findChildren(SymTagNull, NULL, nsNone,
  &pEnumChildren))) { IDiaSymbol *pChild; ULONG celt = 0;

      while (SUCCEEDED(pEnumChildren->Next(1, &pChild, &celt)) && (celt == 1))
  { PrintSymbol(pChild, dwIndent + 2); pChild->Release();
      }
...
bool
DumpAllSymbols (IDiaSymbol *pGlobal)
{
         ...
         && !break_me)
    {
      ...
      if (SUCCEEDED (pCompiland->findChildren (SymTagNull, NULL, nsNone,
                                               &pEnumChildren)))
        {
          ...
          while (SUCCEEDED (pEnumChildren->Next (1, &pSymbol, &celtChildren))
                 && (celtChildren == 1))
            {
...
std::unordered_map<std::wstring, std::wstring> dynarec_cache;
std::unordered_map<std::wstring, unsigned long> dynarec_cache_ln;

auto
get_file_name (const CComPtr<IDiaSourceFile> &source_file,
               bool is_true = false) -> std::wstring
{
  wchar_t *name = L"";

  if (source_file->get_fileName (&name) == S_OK)
    {
      ...
      if (!is_true)
        {
          source_file_name = L"C:/Users/wroy/Desktop/" s;
        }

      source_file_name.append (name);

      // format the invalid characters from wchar_t *name
      std::replace (source_file_name.begin (), source_file_name.end (), '\\',
                    '/');
      source_file_name.erase (
          std::remove (source_file_name.begin () + 23, source_file_name.end (),
                       L':'),
          source_file_name.end ()); // NOLINT(bugprone-unused-return-value,
                                    // clang-diagnostic-unused-result)

      // wchar_t *name is allocated by get_fileName
      SysFreeString (name);

      // return the correct conversion
...
auto
get_line_number (const CComPtr<IDiaSymbol> &function, const std::wstring &name)
{
  auto sym_tag = 0ul;

  if (function->get_symTag (&sym_tag) != S_OK || sym_tag != SymTagFunction)
    return;

  auto sym_length = 0ull;

  if (function->get_length (&sym_length) != S_OK)
    return;

  auto rva = 0ul;

  CComPtr<IDiaEnumLineNumbers> enum_line_numbers;

  if (function->get_relativeVirtualAddress (&rva) == S_OK
      && SUCCEEDED (dia_session->findLinesByRVA (
          rva, static_cast<DWORD> (sym_length), &enum_line_numbers)))
    {
      CComPtr<IDiaLineNumber> line_number;

      auto celt = 0ul;
      unsigned long source_file_id;
      auto source_file_id_last = -1ul;

      while (SUCCEEDED (enum_line_numbers->Next (1, &line_number, &celt))
             && celt == 1)
        {
          auto number = 0ul;

          if (line_number->get_lineNumber (&number) == S_OK
              && line_number->get_sourceFileId (&source_file_id) == S_OK
              && source_file_id != source_file_id_last)
            {
              CComPtr<IDiaSourceFile> source_file;
              line_number->get_sourceFile (&source_file);

              if (auto source_file_name = get_file_name (source_file);
                  !source_file_name.empty ())
                {
                  cache_fn_map.insert (
                      { std::make_pair (number, source_file_name),
                        std::make_pair (dia2dump::lxx, rva) });
                  try
                    {
                      cache_fn_body.insert ({ dia2dump::lxx, vdata });
                    }
                  catch (...)
                    {
                      MessageBoxA (0, "x", "x", MB_OK);
                    }

                  vdata.clear ();

                  cache_fn_body_zy.insert (
                      { dia2dump::lxx, dia2dump::lxx_zy });
                  break;
                }
              // cache.insert({name, std::make_pair(source_file_name,
              // number)});

              source_file_id_last = source_file_id;
              source_file.Release ();
            }
          line_number.Release ();
        }
      enum_line_numbers.Release ();
    }
}
...
get_source_tree ()
{
  CComPtr<IDiaEnumSymbols> enum_compilands;

  if (FAILED (dia_symbol->findChildren (SymTagCompiland, NULL, nsNone,
                                        &enum_compilands)))
    throw;
  ...
  while (SUCCEEDED (enum_compilands->Next (1, &compiland, &celt)) && celt == 1)
    {
      ...
      CComPtr<IDiaEnumSourceFiles> enum_source_files;

      if (SUCCEEDED (dia_session->findFile (compiland, NULL, nsNone,
                                            &enum_source_files)))
        {
          CComPtr<IDiaSourceFile> source_file;

          while (SUCCEEDED (enum_source_files->Next (1, &source_file, &celt))
                 && celt == 1)
            {
              if (auto source_file_name = get_file_name (source_file);
                  !source_file_name.empty ())
                {
                  // create the source directory at the specified location
                  create_directories (
                      std::filesystem::path (source_file_name.c_str ())
                          .parent_path ());

                  // create the source file at the specified location
                  std::ofstream ofs (source_file_name);
                  source_file_name.substr (source_file_name.find_last_of (L".")
                                           + 1)
                          == L"h"
...
cache_fn ()
{
  ...
  while (SUCCEEDED (enum_compilands->Next (1, &compiland, &celt)) && celt == 1)
    {
      CComPtr<IDiaEnumSymbols> enum_functions;

      if (SUCCEEDED (compiland->findChildren (SymTagFunction, NULL, nsNone,
                                              &enum_functions)))
        {
          CComPtr<IDiaSymbol> function;

          while (SUCCEEDED (enum_functions->Next (1, &function, &celt))
                 && celt == 1)
            {
              ...
              if (function->get_name (&name) == S_OK)
                {
                  // if (wcscmp(name, L"Demo_DrawVerticalLine") == 0 ||
                  // wcscmp(name, L"Demo_HandleInput") == 0 || wcscmp(name,
                  // L"Demo_AdjustTimeForPlayerStateHudElement") == 0)
                  // //if(wcscmp(name, L"Demo_DrawVerticalLine") == 0) //
                  //    __debugbreak();

                  // manage both normal functions and function pointers
                  CComPtr<IDiaSymbol> function_type;
                  auto sym_tag = 0ul;

                  if (function->get_type (&function_type) == S_OK)
                    {
                      if (function_type->get_symTag (&sym_tag) == S_OK)
                        {
                          switch (sym_tag)
                            {
                            case SymTagFunctionType:
                              dia2dump::PrintFunctionType (function);
                              break;
                            case SymTagPointerType:
                              dia2dump::PrintFunctionType (function_type);
                              break;
                            default:
                              break;
                            }
                  ...
                  // convert wchar_t* to std::wstring
                  std::wstring function_name (name);

                  // find and sort line numbers
                  get_line_number (function, dia2dump::lxx);

                  // name is allocated by get_fileName
                  SysFreeString (name);
...
main (int argc, char *argv) -> int
{
  ...
  // Initialize DemoA global scope.
  assert (SUCCEEDED (
      NoRegCoCreate (L"C:/Program Files/Microsoft Visual "
                     L"Studio/2022/Preview/DIA SDK/bin/amd64/msdia140.dll",
                     CLSID_DiaSource, IID_IDiaDataSource,
                     reinterpret_cast<void **> (&msdiasdk::dia_data_source))));
  assert (SUCCEEDED (msdiasdk::dia_data_source->loadDataFromPdb (
      CMAKE_INSTALL_PREFIX L"DemoA_O.pdb")));
  assert (SUCCEEDED (
      msdiasdk::dia_data_source->openSession (&msdiasdk::dia_session)));
  assert (SUCCEEDED (
      msdiasdk::dia_session->get_globalScope (&msdiasdk::dia_symbol)));

  std::wcout << "Dumping source tree...\n";

  msdiasdk::get_source_tree ();

  std::wcout << "Caching functions...\n";

  msdiasdk::cache_fn ();

  std::wcout << "Adding functions to source tree...\n";
  std::unordered_map<std::wstring, int> hash;
  for (const auto &entry : cache_fn_map)
    {
      auto key_pair = entry.first;
      const std::string s (entry.second.first.begin (),
                           entry.second.first.end ());
      std::ofstream ofs (key_pair.second, std::ios_base::app);

      if (!hash.count (key_pair.second))
        {
          ofs << "//! @file\n//! SPDX-License-Identifier: "
                 "GPL-3.0-or-later\n\n";
          hash.insert ({ key_pair.second, 0 });
        }

      bool is_defined = false;

      for (auto &local : cache_fn_body_zy)
        {
          bool test = false;

          if (local.first == entry.second.first)
            {
              // check if the same symbol exist in T5.
              CComPtr<IDiaSymbol> symbol;

              // Sometimes findSymbolByRVA returns S_OK, but NULL.
              if (SUCCEEDED (msdiasdk::dia_session->findSymbolByRVA (
                      static_cast<DWORD> (entry.second.second), SymTagFunction,
                      &symbol))
                  && symbol)
                {
                  wchar_t *name = L"";
